    ...
    for image_nbr, gt in enumerate(list_gt):
        ...
        if len(gt) > 0:  # check that ground truth contains at least one annotation
            if len(list_detections_thresh[image_nbr]) > 0:  # if there are detections
                ...
                _, _, _, list_FP, list_FN = computeAssignment(list_detections_thresh[image_nbr], gt, allowedDistance,
                                                              save_disagreement=True)
            else:
                list_FP = []
                list_FN = gt
        ...
        # nbr FP FN
        FNFPallimages.loc[image_id] = pd.Series(
            {'FP': len(list_FP), 'FN': len(list_FN), 'FP+FN': len(list_FP) + len(list_FN)})
...
def computeFROC(proba_map, ground_truth, allowedDistance, nbr_of_thresholds=40, range_threshold=None):
    ...
    for threshold in threshold_list:
        ...
        for i in range(len(proba_map)):
            ...
            if (type(ground_truth) == np.ndarray and np.nonzero(ground_truth) > 0) or (
                    type(ground_truth) == list and len(ground_truth) > 0):
